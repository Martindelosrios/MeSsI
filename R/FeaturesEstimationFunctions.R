...
#{{{
messi <- function(dat, cum = TRUE, gal = TRUE, rank = TRUE, relaxed = TRUE, name.groups = 'estadisticos_grupos.dat', name.gal = 'estadisticos_galaxias.dat', rank.name = 'ranking.dat', relaxed.name = 'ranking_relaxed.dat', folder = 'folder', nrank = 100, name_trainset_cum = 'trainset_cum.dat', name_trainset_gal = 'trainset_gal.dat', ntotal = 0, ngal.lim = 30, est = TRUE, ...){
  ...
  if(cum == TRUE){
    ...
    for(i in 1:ntotal){
      if(length(dat$ra)>0){
        ...
        if(length(dat$id) > 0){ 
          features <- ClusterFeatures(group, ...)
